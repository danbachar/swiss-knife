...
def create_client_socket_and_connect(reverse, udp, ip, port, buffer_length, duration, mss, zerocopy) -> float:
    s = create_socket(udp, ip, buffer_length, mss, zerocopy)
    client_loop(reverse, udp, s, ip, port, buffer_length, duration, zerocopy)
...
def start_server(reverse, udp, host, port, buffer_length, connections, duration, mss, zerocopy) -> int:
    childpid = os.fork()
    if childpid != 0:
        return childpid
    ...
    if connections > 1:
        ...
        (bits,time) = server_loop(reverse, udp, s, buffer_length, duration, zerocopy)
        speed = (bits/time)
...
def start_client(reverse, udp, host, port, buffer_length, duration, connections, mss, zerocopy) -> int:
    ...
    if connections > 1:
        ...
        s = create_socket(udp, ip, buffer_length, mss)
        client_loop(reverse, udp, s, host, port, buffer_length, duration, zerocopy)
    sys.exit(1)
        ...
        if not reverse:
            ...
            while timeModule.time() < end_time:
                ...
                if not zerocopy:
                    ...
                    # print('tell: num of bytes sent:', zerocopyFile.tell())
                    # print('sendfile res: num of bytes sent:', length)
                if firstReceive:
                    firstReceive = False
                    start = timeModule.time()
                received += length
...
def main(argv) -> None:
    try:
        opts, args = getopt.getopt(argv, "p:f:d:c:t:l:uRM:Z",
                                   ["port=" "plot_filename=" "data_filename=" "connections=" "time=" "length=" "udp", "reverse" "set-mss" "zerocopy"])
