...
int main (int argc, char ** argv)
{
    ...
    //temp= temp + 1;   //comment out
    *temp = 20;         //insert a store instruction
    /* out of bounds */
    printf("> main-- alloc[4]: %d <--- no buffer overflow, but reassign a new value\n", *temp); // as we does not increment pointer, so update alloc[5]-> alloc[4]
