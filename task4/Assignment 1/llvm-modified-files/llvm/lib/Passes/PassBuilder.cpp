...
PassBuilder::PassBuilder(TargetMachine *TM, PipelineTuningOptions PTO,
  ...
  if (TM) TM->registerPassBuilderCallbacks(*this);
...
#include "PassRegistry.def"
  ...
  for (auto &C : ModuleAnalysisRegistrationCallbacks) C(MAM);
}
...
#include "PassRegistry.def"
  ...
  for (auto &C : CGSCCAnalysisRegistrationCallbacks) C(CGAM);
}
...
#include "PassRegistry.def"
  ...
  for (auto &C : FunctionAnalysisRegistrationCallbacks) C(FAM);
}
...
#include "PassRegistry.def"
  ...
  for (auto &C : LoopAnalysisRegistrationCallbacks) C(LAM);
}

static Optional<int> parseRepeatPassName(StringRef Name) {
  if (!Name.consume_front("repeat<") || !Name.consume_back(">")) return None;
  int Count;
  if (Name.getAsInteger(0, Count) || Count <= 0) return None;
  return Count;
}

static Optional<int> parseDevirtPassName(StringRef Name) {
  if (!Name.consume_front("devirt<") || !Name.consume_back(">")) return None;
  int Count;
  if (Name.getAsInteger(0, Count) || Count < 0) return None;
  return Count;
}
...
Expected<StackLifetime::LivenessType> parseStackLifetimeOptions(
    StringRef Params) {
...
static bool callbacksAcceptPassName(StringRef Name, CallbacksT &Callbacks) {
  if (!Callbacks.empty()) {
    ...
    for (auto &CB : Callbacks)
      if (CB(Name, DummyPM, {})) return true;
  }
  return false;
}
...
static bool isModulePassName(StringRef Name, CallbacksT &Callbacks) {
  ...
  if (Name == "function" || Name == "function<eager-inv>") return true;

  // Explicitly handle custom-parsed pass names.
  if (parseRepeatPassName(Name)) return true;

#define MODULE_PASS(NAME, CREATE_PASS) \
  if (Name == NAME) return true;
#define MODULE_PASS_WITH_PARAMS(NAME, CLASS, CREATE_PASS, PARSER, PARAMS) \
  if (checkParametrizedPassName(Name, NAME)) return true;
#define MODULE_ANALYSIS(NAME, CREATE_PASS)                           \
...
static bool isCGSCCPassName(StringRef Name, CallbacksT &Callbacks) {
  ...
  // Explicitly handle custom-parsed pass names.
  if (parseRepeatPassName(Name)) return true;
  if (parseDevirtPassName(Name)) return true;
...
static bool isFunctionPassName(StringRef Name, CallbacksT &Callbacks) {
  // Explicitly handle pass manager names.
  if (Name == "function" || Name == "function<eager-inv>") return true;
  if (Name == "loop" || Name == "loop-mssa") return true;
...
PassBuilder::parsePipelineText(StringRef Text) {
  ...
  for (;;) {
    ...
    do {
      ...
      if (PipelineStack.size() == 1) return None;
    ...
    if (!Text.consume_front(",")) return None;
  }
...
Error PassBuilder::parseModulePass(ModulePassManager &MPM,
  ...
  if (!InnerPipeline.empty()) {
    ...
    if (Name == "cgscc") {
      ...
      if (auto Err = parseCGSCCPassPipeline(CGPM, InnerPipeline)) return Err;
  ...
  if (startsWithDefaultPipelineAliasPrefix(Name)) {
    ...
    if (L == OptimizationLevel::O0 && Matches[1] != "thinlto" &&
        Matches[1] != "lto") {
      MPM.addPass(buildO0DefaultPipeline(
          L, Matches[1] == "thinlto-pre-link" || Matches[1] == "lto-pre-link"));
      return Error::success();
    }
...
#define MODULE_PASS_WITH_PARAMS(NAME, CLASS, CREATE_PASS, PARSER, PARAMS) \
  if (checkParametrizedPassName(Name, NAME)) {                            \
    ...
    if (!Params) return Params.takeError();                               \
...
Error PassBuilder::parseCGSCCPass(CGSCCPassManager &CGPM,
  ...
  if (!InnerPipeline.empty()) {
    ...
    for (auto &C : CGSCCPipelineParsingCallbacks)
      if (C(Name, CGPM, InnerPipeline)) return Error::success();
...
Error PassBuilder::parseFunctionPass(FunctionPassManager &FPM,
  ...
  if (!InnerPipeline.empty()) {
    ...
    if (Name == "loop" || Name == "loop-mssa") {
      LoopPassManager LPM;
      if (auto Err = parseLoopPassPipeline(LPM, InnerPipeline)) return Err;
      // Add the nested pass manager with the appropriate adaptor.
    ...
    for (auto &C : FunctionPipelineParsingCallbacks)
      if (C(Name, FPM, InnerPipeline)) return Error::success();
...
Error PassBuilder::parseLoopPass(LoopPassManager &LPM,
  ...
  if (!InnerPipeline.empty()) {
    ...
    for (auto &C : LoopPipelineParsingCallbacks)
      if (C(Name, LPM, InnerPipeline)) return Error::success();
...
Error PassBuilder::parseLoopPassPipeline(LoopPassManager &LPM,
  ...
  for (const auto &Element : Pipeline) {
    if (auto Err = parseLoopPass(LPM, Element)) return Err;
  }
  return Error::success();
}
...
Error PassBuilder::parseFunctionPassPipeline(
  ...
  for (const auto &Element : Pipeline) {
    if (auto Err = parseFunctionPass(FPM, Element)) return Err;
  }
  return Error::success();
}
...
Error PassBuilder::parseCGSCCPassPipeline(CGSCCPassManager &CGPM,
                                          ArrayRef<PipelineElement> Pipeline) {
  for (const auto &Element : Pipeline) {
    if (auto Err = parseCGSCCPass(CGPM, Element)) return Err;
  }
  return Error::success();
}
...
Error PassBuilder::parseModulePassPipeline(ModulePassManager &MPM,
                                           ArrayRef<PipelineElement> Pipeline) {
  for (const auto &Element : Pipeline) {
    if (auto Err = parseModulePass(MPM, Element)) return Err;
  }
  return Error::success();
}
  ...
  if (!isModulePassName(FirstName, ModulePipelineParsingCallbacks)) {
    ...
    if (isCGSCCPassName(FirstName, CGSCCPipelineParsingCallbacks)) {
      ...
      for (auto &C : TopLevelPipelineParsingCallbacks)
        if (C(MPM, *Pipeline)) return Error::success();
...
Error PassBuilder::parsePassPipeline(CGSCCPassManager &CGPM,
  ...
  if (auto Err = parseCGSCCPassPipeline(CGPM, *Pipeline)) return Err;
  return Error::success();
}
...
Error PassBuilder::parsePassPipeline(FunctionPassManager &FPM,
  ...
  if (auto Err = parseFunctionPassPipeline(FPM, *Pipeline)) return Err;
  return Error::success();
}
...
Error PassBuilder::parsePassPipeline(LoopPassManager &CGPM,
  ...
  if (auto Err = parseLoopPassPipeline(CGPM, *Pipeline)) return Err;
