...
namespace {
    ...
    static Function* MyMalloc = nullptr;
    ...
    class BoundsCheckerModule : public ModulePass {
        public:
            ...
            virtual bool runOnModule(Module& M) {
                ...
                SmallVector <Value*, 0> PointerOperandList;
                    ...
                    for (auto BI= F->begin(); BI!= F->end(); ++BI) {
                        for (auto II= BI->begin(); II!=BI->end(); ++II) {
                            ...
                            if (auto * LI= dyn_cast<LoadInst>(Ins)) {
                                ...
                                DuplicateFlag = false;
                                for (auto PO= PointerOperandList.begin(); PO!= PointerOperandList.end(); ++PO) {
                                    if(*PO == LI->getPointerOperand())
                                    {
                                        DuplicateFlag = true;
                                        break;
                                    }
                                ...
                                if(!DuplicateFlag){
                                    PointerOperandList.push_back(LI->getPointerOperand());
                            ...
                            /*  Instrument a StoreInst (memory write) */
                                ...
                                if(!DuplicateFlag){
                                    PointerOperandList.push_back(SI->getPointerOperand());
